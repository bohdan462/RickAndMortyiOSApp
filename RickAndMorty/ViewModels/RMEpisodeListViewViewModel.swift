...
final class RMEpisodeListViewViewModel: NSObject {
    ...
    private var episodes: [RMEpisode] = [] {
        didSet {
            for episode in episodes {
                let viewModel = RMCharacterEpisodesCollectionViewCellViewModel(episodeDataUrl: URL(string:episode.url), borderColor: borderColors.randomElement() ?? .systemBlue)
    ...
    ///Peginate if additional episodes are needed
    public func fetchAdditionalEpisodes(url: URL) {
        ...
        RMService.shared.execute(request, expecting: RMGetAllEpisodesResponse.self, complition: { [weak self] result in
            ...
            case .success(let responseModel):
                ...
                let orifinalCount = strongSelf.episodes.count
                let newCount = moreResults.count
                let total = orifinalCount + newCount
...
extension RMEpisodeListViewViewModel: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.2, repeats: false, block: { [weak self] t in
            ...
            let totalScrollViewFixedHeaight = scrollView.frame.size.height
            
            if offset >= (totalContentHeight - totalScrollViewFixedHeaight - 120) {
